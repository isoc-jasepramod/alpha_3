"""
PROJECT ALPHA 3.0 — YESTERDAY BACKTEST (2026-09-24 THURSDAY)
Validates all strategy engines on real 1-minute historical data from AngelOne:
Thursday is NIFTY 50 Weekly Expiry Day (0-DTE Gamma Scalp)
1. Volume-Backed ORB (09:15 - 10:30)
2. VWAP & EMA Alignment (09:15 - 15:15)
3. Expiry Day Gamma Scalp (13:15 - 15:30) [Thursday = NIFTY 0-DTE Expiry]
4. Momentum Impulse Detector (09:20 - 15:20)
5. Precursor Radar Engines (GEX & IV Skew)
"""
...
def fetch_historical_candles(date_str="2026-09-24") -> Dict[str, pd.DataFrame]:
...
def simulate_gamma_scalp(df: pd.DataFrame, inst: str, weekday: int) -> Optional[Dict[str, Any]]:
    """
    Simulates Expiry-Day Gamma Scalp (13:15 - 15:30).
    Thursday (weekday 3) is NIFTY Expiry Day!
    Friday (weekday 4) is SENSEX Expiry Day!
    """
    ...
    # Consolidation filter: Range <= 0.3% of spot (~70 pts NIFTY, ~220 pts SENSEX)
...
def run_backtest_yesterday():
    date_str = "2026-09-24"
    weekday = datetime.strptime(date_str, "%Y-%m-%d").weekday() # 3 = Thursday (NIFTY Expiry)

    print("=" * 90)
    print(f"  PROJECT ALPHA 3.0 | YESTERDAY BACKTEST ({date_str} THURSDAY)")
    print("  NIFTY 50 Weekly Expiry Day (0-DTE Gamma Scalp & Afternoon Squeeze)")
    print("  Real 1-Minute Historical Data from AngelOne SmartConnect")
    print("=" * 90)

    candles_map = fetch_historical_candles(date_str)
    if not candles_map:
        print(f"Failed to retrieve candles for {date_str}.")
        return
    ...
    for inst in ["NIFTY", "SENSEX"]:
        ...
        # 2. Expiry Day Gamma Scalp (NIFTY on Thursday)
        gamma_trade = simulate_gamma_scalp(df, inst, weekday)
        if gamma_trade:
